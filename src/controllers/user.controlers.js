...
const isPasswordStrong = (password) => {
  ...
  if (password.length > 16) {
    throw new ApiErrorResponse(
      401,
      "Password must not be more than 16 characters long",
    );
  }
...
const registerUser = AsyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(201)
      .json(new ApiResponse(201, "User registered successfully", { dbUser }));
  } catch (error) {
...
const loginUser = AsyncHandler(async (req, res) => {
  ...
  if ([email, password].some((field) => field === "")) {
    throw new ApiErrorResponse(
      401,
      "Both email and password is required for user login",
    );
  }
  ...
  const passwordMatched = await user.isPasswordMatching(password);
  if (!passwordMatched) {
    throw new ApiErrorResponse(401, "Enter a valid password");
  }
  try {
    //generate Refresh and access Token
    const { refreshToken, accessToken } = await gernerateRefreshAndAccessToken(
      user._id,
    );
    const options = {
      httpOnly: true,
...
const deleteProfileImage = AsyncHandler(async (req, res) => {
  try {
    ...
    return res.status(201).json(new ApiResponse(201, "Successfull", publicId));
  } catch (error) {
...
const updateProfileImage = AsyncHandler(async (req, res) => {
  ...
  try {
    ...
    throw new ApiErrorResponse(
      201,
      "Something went wrong while updaing the profile photo",
      error,
    );
  }
});
...
const getAllUser = AsyncHandler(async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json(
        new ApiResponse(201, "all the user fetched successfully", allUsers),
      );
  } catch (error) {
    console.log(error);
    throw new ApiErrorResponse(
